...
impl BlendModeBuilder {
    ...
    /// when composed and used.
    ///
    /// However, `BlendModeBuilder` only ever outputs [`BlendMode::Custom`]
    /// when composed, even if the produced blend mode was functionally
    ...
    ///
    /// assert_eq!(bmb1,bmb2);
    /// assert_eq!(bmb1.compose(),bmb2.compose());
    /// ```
...
pub enum BlendMode {
    ...
    Invalid,
    /// Custom blend mode, composed using [`BlendModeBuilder::compose`].
    Custom(BlendValue),
}
